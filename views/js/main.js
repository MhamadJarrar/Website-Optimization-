...
// Chooses random adjective and random nounn
function randomName() {
...
// times is the array of User Timing measurements from updatePositions()
  var NoOfEntries = times.length;
  var sum = 0;
 
  for (var i = NoOfEntries - 1; i > NoOfEntries - 11; i--) {
...
function updatePositions() {
  ...
  var point = [];
  for (var i = 0; i < 5; i++) {
    point.push(Math.sin(topPosition + i));
  }

  var count=0;
  for (count = 0; count < slides.length; count++) {
    slides[count].style.left = slides[count].basicLeft + 100 * point[count % 5] + 'px';
  }
  ...
  if (frame % 10 === 0) {
    var timeOfUpdate = window.performance.getEntriesByName("measure_frame_duration");
    logAverageFrame(timeOfUpdate);
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < pizzasDisplay; i++) {
    var element = document.createElement('img');

    element.className = 'mover';
    element.src = "images/pizza.png";
    element.style.height = "100px";
    element.style.width = "73.333px";
    element.basicLeft = (i % cols) * s;
    element.style.top = (Math.floor(i / cols) * s) + 'px';
    movingPizzas.appendChild(element);
   // document.querySelector("#movingPizzas1").appendChild(element);
  }
  slides = document.getElementsByClassName('mover');
  updatePositions();
});
